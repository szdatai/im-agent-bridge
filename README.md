...
| 场景 | 直接发送 |
|---|---|
| 项目结构 | 列出 <项目> 顶层目录结构,标注每个目录的用途 |
| 最近提交 | 查看 git 最近 10 条提交,简述每条改动 |
| 工作区状态 | git status 并汇总未提交改动 |
| 跑测试 | 找到并运行核心模块测试,报告结果 |
| 语法检查 | 对 <文件> 做语法/静态检查 |
| 新增字段 | 在 <DocType> 增加字段 <field>,补迁移并注册 |
| 修改逻辑 | 在 <文件> 修改 <逻辑>,说明你的改动 |
| 查报错 | 查看最近日志里的报错,分析原因 |
| 修 bug | <现象描述>,帮我定位根因并修复,最后跑相关测试确认 |
| 代码审查 | 审查 <分支/提交> 的改动,给出问题清单 |
| 执行命令 | 运行 <命令> 并解释输出 |
| 解释代码 | 解释 <文件> 的作用和关键逻辑 |

**使用技巧**:
- 指令越具体越好(指明文件/路径/验收标准),agent 执行越准;
- 改完代码建议追加一句「跑一下相关测试确认」;
- 发图/文件后 30 秒内补一句指令,agent 会读取附件;
- 想操作工作目录外的路径,在指令里给绝对路径即可。
...
| `WECHAT_*` | 微信 iLink baseUrl + 账号目录 + 白名单(`WECHAT_OPEN=1` 默认开放,`0`=需白名单) |
